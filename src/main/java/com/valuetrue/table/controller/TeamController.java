...
public class TeamController {
    ...
    public @ResponseBody
    boolean checkTeamName(String nameCheck, int teamIdCheck) {
        // Get all Teams
        List<Team> listTeams = this.teamService.getAllTeams();
        for(int i = 0; i < listTeams.size();i++){
            if(nameCheck.trim().equalsIgnoreCase(listTeams.get(i).getName().trim()) && teamIdCheck == listTeams.get(i).getTeamId() ){
                return false;
            }
        }
